	...
	<div class="col-md-8 cart-content-table">
				...
				foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
					...
					if ( $_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters( 'woocommerce_cart_item_visible', true, $cart_item, $cart_item_key ) ) {
						...
						<tr class="<?php echo esc_attr( apply_filters( 'woocommerce_cart_item_class', 'cart_item', $cart_item, $cart_item_key ) ); ?>">
							...
							<td class="product-name strong" data-title="<?php _e( 'Product', 'woocommerce' ); ?>">
								<?php
									if ( ! $product_permalink ) {
										...
										echo apply_filters( 'woocommerce_cart_item_name', sprintf( '<a class="orion-product-title" href="%s">%s</a>', esc_url( $product_permalink ), $_product->get_title() ), $cart_item, $cart_item_key );
									}
								...
								<div class="product-remove">
									<?php
										echo apply_filters( 'woocommerce_cart_item_remove_link', sprintf(
											'<a href="%s" class="remove black-hover" title="%s" data-product_id="%s" data-product_sku="%s"><i class="fa fa-times"></i></a>',
							...
							<td class="product-subtotal font-2" data-title="<?php _e( 'Total', 'woocommerce' ); ?>">
								<?php
		...
		<div class="row">
			<div class="col-sm-12 update-cart-btn icon-right text-right">
				<button type="submit" class="btn btn-empty btn-md btn-c3" name="update_cart" value="<?php esc_attr_e( 'Update Cart', 'woocommerce' ); ?>"> 
			...
			<div class="col-sm-12">
				...
				<?php if ( wc_coupons_enabled() ) { ?>
					<div class="coupon row">
						...
						<div class="col-sm-4">
						  <input type="submit" class="btn btn-md btn-flat btn-c3 btn-block block" name="apply_coupon" value="<?php esc_attr_e( 'Apply Coupon', 'woocommerce' ); ?>" />
						</div>
