...
class category_posts extends WP_Widget {
	function __construct() {
		/* Widget settings. */
		$widget_ops = array( 'classname' => 'category_posts', 'description' => 'Displays the post image and excerpt from a selected category **Does not work on singlepost sidebar**'.'ecorecycle' );
		/* Create the widget. */
	...
	function widget( $args, $instance ) {
		...
		if ($pc->have_posts()) : 
			...
			echo pmc_security($before_widget); 
		
				if ( $title ) echo pmc_security($before_title . $title . $after_title); 
		?>
	...
	?>			<div class="imgholder">
					<a href="<?php the_permalink() ?>" rel="bookmark" title="Permanent Link to <?php the_title(); ?>">
						<?php if (has_post_thumbnail( get_the_ID() )) echo '<img src = '.esc_url($image).' alt = "'.esc_attr(get_the_title() ? get_the_title() : get_the_ID()).'"  > ' ;?>	
					</a>
				</div>
