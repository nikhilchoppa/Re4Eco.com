...
var tld_editorCSS;
var tld_targetStepID = '';
var tld_targetDomElement = '';
...
function tld_onOpen() {
    ...
    if (jQuery('#tld_tdgnFrame').attr('src') == 'about:blank') {
        //||(tld_targetStepID == "" || jQuery('#tld_tdgnFrame').contents().find('#estimation_popup .genSlide[data-stepid="' + tld_targetStepID + '"]').length == 0)
...
function tld_tdgn_init() {
    ...
    jQuery('#tld_tdgnFrame').on('load', function () {
        if (jQuery('#tld_tdgnFrame').attr('src') != 'about:blank') {
            ...
            if (tld_targetStepID != '') {
                ...
                jQuery('#tld_tdgnFrame').get(0).contentWindow.lfb_setAnimImmediate(lfb_currentFormID);


                jQuery('#tld_tdgnFrame').contents().find('#estimation_popup').attr('data-nowstart', 1);
                jQuery('#tld_tdgnFrame').contents().find('#estimation_popup').attr('data-animspeed', 0);
                jQuery('#tld_tdgnFrame').contents().find('.lfb_activeStep').removeClass('lfb_activeStep');
                jQuery('#tld_tdgnFrame').contents().find('[data-start="1"]').attr('data-start', '0');
                jQuery('#tld_tdgnFrame').contents().find('.genSlide[data-stepid="' + tld_targetStepID + '"]').addClass('lfb_activeStep');
                jQuery('#tld_tdgnFrame').contents().find('.genSlide[data-stepid="' + tld_targetStepID + '"]').attr('data-start', '1');

                jQuery('#tld_tdgnFrame').contents().find('#estimation_popup .lfb_startBtnContainer').hide();
                jQuery('#tld_tdgnFrame').contents().find('#estimation_popup #genPrice').show();
                jQuery('#tld_tdgnFrame').contents().find('#estimation_popup #mainPanel').attr('style', 'transition: none !important;opacity:1 !important;display: block !important;');
                jQuery('#tld_tdgnFrame').contents().find('.lfb_activeStep').show().attr('style', 'transition: none !important;opacity:1 !important;display: block !important;');
                jQuery('#tld_tdgnFrame').contents().find('.lfb_activeStep .genContent').show().attr('style', 'transition: none !important;opacity:1 !important;display: block !important;');
                jQuery('#tld_tdgnFrame').contents().find('.lfb_activeStep .stepTitle').show().attr('style', 'transition: none !important;opacity:1 !important;display: block !important;');
                jQuery('#tld_tdgnFrame').contents().find('.lfb_activeStep .stepTitle').addClass('positionned');
                jQuery('#tld_tdgnFrame').contents().find('.lfb_activeStep .datetimepicker').hide();
                jQuery('#tld_tdgnFrame').contents().find('.lfb_activeStep .errorMsg ').hide();
                jQuery('#tld_tdgnFrame').contents().find('.lfb_activeStep .tooltip').hide();
                jQuery('#tld_tdgnFrame').contents().find('.lfb_activeStep .btn-next').show();

                jQuery('#tld_tdgnFrame').get(0).contentWindow.lfb_resizeAll();

                if (tld_targetDomElement != '') {
                    tld_selectElement(jQuery('#tld_tdgnFrame').contents().find(tld_targetDomElement));
                    tld_elementInitialized = false;
                    tld_getElementStyles(jQuery('#tld_tdgnFrame').contents().find(tld_targetDomElement)[0]);
                    setTimeout(function () {
                        tld_elementInitialized = true;
                    }, 250);
                }
                jQuery('#tld_tdgnFrame').contents().find('#mainPanel .genSlide[data-stepid="' + tld_targetStepID + '"] .lfb_rate').each(function () {
    ...
    jQuery('#tld_tdgnPanel').find('select').on('change', tld_saveCurrentElement);
...
function tld_getPath(el, modeClasses, start) {
    ...
    if (jQuery(el).length > 0 && typeof (jQuery(el).prop('tagName')) != "undefined") {
        if (jQuery(el).attr('id') != 'estimation_popup') {
            ...
            if ((start && modeClasses && (!jQuery(el).attr('id') || jQuery(el).attr('id') != 'wpe_panel')) || !jQuery(el).attr('id') || jQuery(el).attr('id').substr(0, 9) == 'ultimate-' || jQuery(el).attr('id').substr(0, 6) == 'ui-id-' || jQuery(el).attr('id').substr(0, 2) == 'dp') {
                if (!modeClasses) {
                    if (jQuery(el).is('[data-id]')) {
                        path = '> [data-id="' + jQuery(el).attr('data-id') + '"]';
                    } else if (jQuery(el).is('[data-stepid]')) {
                        path = '> [data-stepid="' + jQuery(el).attr('data-stepid') + '"]';
                    } else {
                        ...
                        if (jQuery(el).is('.genSlide')) {
                        ...
                        } else if (jQuery(el).is('[class]')) {
                            var classes = '.' + jQuery(el).attr('class');
                            if (classes.indexOf(' ') > -1) {
                                classes = jQuery(el).attr('class').replace(/  /g, ' ');
                            ...
                            classes = classes.replace(/.tld_edited/g, '');
                            classes = classes.replace(/.tld_selectedElement/g, '');
                            if (classes != '') {
                                target = '> ' + classes;
                            }
                        }
                    ...
                    path = jQuery('#tld_tdgn_applyToClasses').val();
                    var classes = "";
    ...
    path = path.replace(/.wpe_bootstraped/g, '');
    return path;
}
