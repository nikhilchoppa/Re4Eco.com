...
function nta_whatsapp_activation_hook()
{
    $option = get_option('nta_whatsapp_review');
    if ($option) {
        update_option('nta_whatsapp_review', 'show'); //Show now
    } else {
        update_option('nta_whatsapp_review', time() + 2 * 60 * 60 * 24); //After 2 days show
    }
}

register_uninstall_hook(__FILE__, 'nta_whatsapp_uninstall_hook');
register_activation_hook(__FILE__, 'nta_whatsapp_activation_hook');
