...
class Settings {
	...
	public function after_saved_hook() {
		...
		if ( array_key_exists( "{$this->prefix}_sanitize_ongoing", $_COOKIE ) && 'yes' == $_COOKIE[ $this->prefix . '_sanitize_ongoing' ] ) {
			add_action( 'admin_notices', function () {
				echo '<div id="setting-error-pwa_settings_updated" class="notice notice-success is-dismissible">';
				echo '<p><strong>' . __( 'Settings Saved.', 'pwa' ) . '</strong></p>';
				echo '</div>';
			} );
			...
			add_submenu_page( $this->parent_page, $name, $name, $this->capability, $key, function () use ( $name, $key ) {
				...
				<div class="wrap <?php echo $this->prefix; ?>-wrap <?php echo $key; ?>-wrap">
					...
					<div class="<?php echo $this->prefix; ?>-wrap__content">
						...
						?>
						<form method="post" action="options.php" id="<?php echo $key; ?>">
							<?php
	...
	private function do_field( $key, $type, $choices = [], $args = [] ) {
		...
		switch ( $type ) {
			case 'checkbox':
				$return .= sprintf( '<input type="hidden" value="0" name="%1$s[%2$s]"/><input type="checkbox" class="pwa-original-checkbox" name="%1$s[%2$s]" value="1" id="%2$s" %3$s /><label for="%2$s" class="pwa-checkbox"><span><svg width="10px" height="10px" viewBox="0 0 10 10"><path d="M5,1 L5,1 C2.790861,1 1,2.790861 1,5 L1,5 C1,7.209139 2.790861,9 5,9 L5,9 C7.209139,9 9,7.209139 9,5 L9,5 C9,2.790861 7.209139,1 5,1 L5,9 L5,1 Z"></path></svg></span></label>', $this->option_key, $key, ( true == $val ? 'checked' : '' ) );
				break;
			case 'select':
			...
			case 'textarea':
				...
				break;
			case 'number':
				$return .= sprintf( '<input type="number" min="1" name="%1$s[%2$s]" id="%2$s" value="%3$s" required />', $this->option_key, $key, $val );
				break;
			case 'color':
			...
			case 'file':
				...
				$return .= '<a class="button button--pwa-settings-delete delete-file">Delete</a>';
				$return .= '</p>';
	...
	public function add_number( $section, $key, $name, $default = '', $args = [] ) {
		if ( ! isset( $this->sections[ $section ] ) ) {
			return false;
		}
